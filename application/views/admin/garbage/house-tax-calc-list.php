                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <small class="float-end">
                            <a href="<?= base_url('Garbage/calculateGarbage'); ?>" class="btn btn-primary">
                                वापस जाएं
                            </a>
                        </small>
                    </div>
                    <form action="<?= base_url('Garbage/saveDemandTax'); ?>" method="POST">
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $billno     = $this->db->select('billno')->from('garbage_demand')->order_by('billno', 'desc')->get()->row();
                                        if(empty($billno)){ $num = 0; }else{$num=$billno->billno;}
                                        $i=0; foreach ($assesseeLists as $key => $value) {                                        
                                            $nill_arv = $this->db->get_where('garbage_registrations', ['crn'=>$value['crn']], ['fin_year' => $finYear])->result();
